...
enum class OneTapMode : uint8_t { Dice, Coin, Fortune, RandomNews, Weather, RandomStock, Decision, Count };
OneTapMode oneTapMode = OneTapMode::Dice;
String oneTapTitle="骰子", oneTapMain="按 A", oneTapDetail="短按A触发 · 长按A换玩法";
...
String htmlEscape(const String&s){String o;for(size_t i=0;i<s.length();++i){char c=s[i];if(c=='&')o+="&amp;";else if(c=='<')o+="&lt;";else if(c=='>')o+="&gt;";else if(c=='\"')o+="&quot;";else o+=c;}return o;}
String setupPageHtml(){String p;p.reserve(2600);p+=F("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Pocket Nexus</title><style>body{font-family:system-ui;background:#0b0d10;color:#fff;padding:24px}main{max-width:520px;margin:auto;background:#151920;padding:24px;border-radius:18px}input,button{box-sizing:border-box;width:100%;font-size:16px;padding:12px;margin:7px 0;border-radius:10px}input{background:#0f1318;color:white;border:1px solid #39434f}button{border:0;background:#35d0ba;color:#07110f;font-weight:700}label{display:block;color:#aeb8c5;margin-top:10px}p{color:#aeb8c5;line-height:1.5}</style></head><body><main><h2>Pocket Nexus 配置</h2><p>仅在临时安全热点中开放，设置保存在设备本机。</p><form method='post' action='/save'><input type='hidden' name='token' value='");p+=setupToken;p+=F("'><label>2.4 GHz Wi-Fi</label><input name='ssid' maxlength='32' required value='");p+=htmlEscape(configuredSsid);p+=F("'><input name='password' type='password' maxlength='63' placeholder='留空保留原密码'><label>最近事件标题</label><input name='event_title' maxlength='48' placeholder='例如：例行洁牙' value='");p+=htmlEscape(eventTitle);p+=F("'><label>事件时间</label><input name='event_time' type='datetime-local'><button type='submit'>保存并重启</button></form><p>热点最多 1 台设备，10 分钟自动关闭。</p></main></body></html>");return p;}
...
void nextOneTapMode(){uint8_t n=(static_cast<uint8_t>(oneTapMode)+1)%static_cast<uint8_t>(OneTapMode::Count);oneTapMode=(OneTapMode)n;oneTapMain="按 A";oneTapDetail="短按A触发 · 长按A换玩法";renderOneTap();}
void triggerOneTap(){
  ...
  static const char* decisions[]={"可以","先等等","不建议","今天就做","换个方案","值得试一次","睡一觉再决定","问问另一个人"};
  ...
  switch(oneTapMode){
    ...
    case OneTapMode::Decision:{const char* d=decisions[esp_random()%(sizeof(decisions)/sizeof(decisions[0]))];oneTapMain=d;oneTapDetail="把它当成打破纠结的小推力";break;}
    default:break;
  }
  renderOneTap();
}
...
void enterSelected(){current=(AppId)selected;inApp=true;pageIndex=0;if(current==AppId::Market){if(!stocks[0].valid)fetchStocks();if(newsCount==0)fetchNews();}if(current==AppId::IR)startIr();if(current==AppId::Level&&!levelCalibrated)calibrateLevel();if(current==AppId::Tilt){gameX=W()/2.0f;gameY=H()/2.0f;gameVx=gameVy=0;}renderCurrent();}
void leaveApp(){if(current==AppId::IR)stopIr();inApp=false;pageIndex=0;renderMenu();}
...
void periodicRefresh(){if(setupPortalActive)return;uint32_t now=millis();if(!screensaverActive&&!irActive&&now-lastInteractionAt>=SCREENSAVER_TIMEOUT_MS){activateSaver();return;}if(screensaverActive||!inApp)return;if(current==AppId::IR){if(pollIr())renderIR();return;}if(current==AppId::Tilt&&now-lastRender>=45){renderTilt();return;}if(current==AppId::Level&&now-lastRender>=90){renderLevel();return;}if((current==AppId::Info||current==AppId::Schedule||current==AppId::Settings)&&now-lastSecond>=1000){lastSecond=now;renderCurrent();}}
